...
namespace SSDQopenECA.Model
{
    ...
    public class Unmapper : UnmapperBase
    {
        ...
        private SSDQopenECA.Model.GPA._Out_DataMeta FillGPA_Out_DataMeta(TypeMapping typeMapping)
        {
            ...
            //Modified by Tapas for dynamic Mapping 
            int count = Algorithm.SSDQ_config.Outentrynamelist.Count();
            for (int i = 0; i < count; i++)
            {
                FieldMapping fieldMapping = fieldLookup[Algorithm.SSDQ_config.Outentrynamelist[i]];
                obj.GetType().GetProperty(Algorithm.SSDQ_config.Outentrynamelist[i]).SetValue(obj, CreateMetaValues(fieldMapping));
        ...
        private void CollectFromGPAOut_Data(List<IMeasurement> measurements, TypeMapping typeMapping, SSDQopenECA.Model.GPA.Out_Data data, SSDQopenECA.Model.GPA._Out_DataMeta meta)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                IMeasurement measurement = MakeMeasurement((MetaValues)meta.GetType().GetProperty(Algorithm.SSDQ_config.Outentrynamelist[i]).GetValue(meta, null), (double)data.GetType().GetProperty(Algorithm.SSDQ_config.Outentrynamelist[i]).GetValue(data, null));
