...
namespace HankelRobustDataEstimation
{
    ...
    public class HankelProcessComplex
    {
        ...
        public HankelProcessComplex(int Meas)
        {
            ...
            threshold[0] = Vector<double>.Build.Dense(num_channel, 0.0015);
            threshold[1] = Vector<double>.Build.Dense(num_channel, 0.0052);
        ...
        public Matrix<double>[] ProcessFrame(Vector<double>[] Current_data, int numberOfFrame)
        {
            ...
            Matrix<double> estimated_mag;
            Matrix<Complex> estimated_ang;
            ...
            if (!Init_flag)
            {
                if (numberOfFrame == window_size)
                {
                    ...
                    estimated_mag = Matrix_Magnitude(data_updated);
                    estimated_ang = Matrix_Phase(data_updated);
            ...
            {
                Matrix<Complex> corrected = Programe.SAP(data_observed.SubMatrix(0, num_channel, 0, window_size), Hankel_k, 1, Math.Pow(10, -3)); //correct data
            ...
            estimated_mag = Matrix_Magnitude(data_estimate);
            estimated_ang = Matrix_Phase(data_estimate);
            ...
            else
            {
                ...
                if (bad_channels != 0)
                {
                    ...
                    for (int jj = 0; jj < bad_channels; jj++)
                    {
                        ...
                        for (int i = 0, ii = jj; i < num_rows; i++, ii += num_channel)
                        {
