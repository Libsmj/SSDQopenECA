...
namespace HankelRobustDataEstimation
{
    ...
    public class hankelRobustDataEstimation
    {
        ...
        public void Initialize(int Meas)
        {
            ...
            data_estimate = Matrix<double>.Build.Dense(Convert.ToInt32(num_channel), windowSize + 1);
            data_updated = Matrix<double>.Build.Dense(Convert.ToInt32(num_channel), windowSize);
            data_observed = Matrix<double>.Build.Dense(Convert.ToInt32(num_channel), windowSize + 1);
        ...
        public Matrix<double> ProcessFrame(Vector <double> Current_data ,int numberOfFrame)
        {
            ...
            {
                Matrix<double> corrected = Programe.SAP(data_estimate.SubMatrix(0, num_channel, 0, windowSize), windowSize / 2, 1, Math.Pow(10, -3)); //correct data is provided in form of a "corrected matrix"
            ...
            if (numberOfFrame >= windowSize && Init_flag)
            {
                ...
                Matrix<double> Hankel_matrix = ExtensionFunction.ExtensionFunction.Hankel(data_estimate.SubMatrix(0, num_channel, 0, windowSize), Hankel_k);
                // Hankel matrix of past L observations
                Matrix<double> ct_kvectors = ExtensionFunction.ExtensionFunction.Hankel(data_estimate.SubMatrix(0, num_channel, windowSize - Hankel_k + 1, Hankel_k), Hankel_k);
                ...
                {
                    flag_event--;
                    Matrix<double> window_data = data_observed.SubMatrix(0, num_channel, 0, windowSize);
            ...
            {
                Matrix<double> corrected = Programe.SAP(data_estimate.SubMatrix(0, num_channel, 0, windowSize), windowSize / 2, 1, Math.Pow(10, -3)); //correct data
            ...
            if (Init_flag) //this line add by Hongyun and Lin, make sure does not oupt anything before correct the first window data
            {
                ...
                else
                {
                    ...
                    Matrix<double> corrected = Programe.SAP(data_observed.SubMatrix(0, num_channel, 0, windowSize), windowSize / 2, 3, Math.Pow(10, -3)); // correct data
