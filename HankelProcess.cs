...
namespace HankelRobustDataEstimation
{
    ...
    public class HankelProcess
    {
        ...
        public HankelProcess(int Meas)
        {
            ...
            threshold = Vector<double>.Build.Dense(num_channel);
        ...
        public Matrix<double> ProcessFrame(Vector<double> Current_data, int numberOfFrame)
        {
            ...
            if (!Init_flag && numberOfFrame >= window_size)
            {
                ...
                if (flag_data_type == 2)
                {
                    ...
                    for (int i = 0; i < num_channel; i++)
                    {
                        threshold[i] = Math.Max(ArrayStatistics.StandardDeviation(data_updated.Row(i).ToArray()), 0.001) / 1.5;
                    }
                }
                else if (flag_data_type == 3)
                {
                    Matrix<double> diff = data_updated.SubMatrix(0, num_channel, 0, window_size - 1) - data_updated.SubMatrix(0, num_channel, 1, window_size - 1);
                    Vector<double> mean = Vector<double>.Build.Dense(num_channel);
                    for (int i = 0; i < num_channel; i++)
                    {
                        threshold[i] = ArrayStatistics.Mean(diff.Row(i).ToArray());
                    }
                    ratio_approx_error = 1.5 * ratio_approx_error;
                }
                else if (flag_data_type == 4)
                {
                    Matrix<double> diff = data_updated.SubMatrix(0, num_channel, 0, window_size - 1) - data_updated.SubMatrix(0, num_channel, 1, window_size - 1);
                    Vector<double> mean = Vector<double>.Build.Dense(num_channel);
                    for (int i = 0; i < num_channel; i++)
                    ...
                    threshold = Vector<double>.Build.Dense(num_channel, threshold.Mean() * 5 / 3);
                }
                else
                {
                    throw new Exception("Data type not set to a valid value : " + flag_data_type.ToString());
                }
            ...
            if (Init_flag && numberOfFrame > window_size)
            {
                ...
                else
                {
                    ...
                    if (bad_channels != 0)
                    {
                        ...
                        {
                            Matrix<double> Hankel_chanel = Matrix<double>.Build.Dense(num_rows, Hankel_matrix_2.ColumnCount);
                            for (int i = 0, ii = jj; i < Hankel_chanel.RowCount; i++, ii += num_channel)
                            {
                                Hankel_chanel.SetRow(i, Hankel_matrix_2.Row(ii));
                            }
