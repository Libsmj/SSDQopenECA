...
namespace HankelRobustDataEstimation
{
    ...
    public class hankelRobustDataEstimation
    {
        //keep the past L estimated data and observed data
        private  Matrix<double> submatrix;               // contain the past L estimations
        private  Matrix<double> submatrix_raw_data;      // contains the past L raw observations        
        ...
        private  Matrix<double> submatrix_updated;       // submatrix of last window size of data for all the channels(introduced for openECA implementation)
        ...
        public void Initialize(int Meas)
        {
            ...
            submatrix = Matrix<double>.Build.Dense(Convert.ToInt32(num_channel), wdsize + 1);
            submatrix_updated = Matrix<double>.Build.Dense(Convert.ToInt32(num_channel), wdsize);
            submatrix_raw_data = Matrix<double>.Build.Dense(Convert.ToInt32(num_channel), wdsize + 1);
        ...
        public Matrix<double> ProcessFrame(Vector <double> Current_data ,int numberOfFrame)
        {
            ...
            {
                Matrix<double> corrected = Programe.SAP(submatrix.SubMatrix(0, num_channel, 0, wdsize), wdsize / 2, 1, Math.Pow(10, -3)); //correct data is provided in form of a "corrected matrix"
            ...
            if (numberOfFrame >= wdsize && Init_flag)
            {
                ...
                Matrix<double> Hankel_matrix = ExtensionFunction.ExtensionFunction.Hankel(submatrix.SubMatrix(0, num_channel, 0, wdsize), Hankel_k);
                // Hankel matrix of past L observations
                Matrix<double> ct_kvectors = ExtensionFunction.ExtensionFunction.Hankel(submatrix.SubMatrix(0, num_channel, wdsize - Hankel_k + 1, Hankel_k), Hankel_k);
                ...
                {
                    flag_event--;
                    Matrix<double> window_data = submatrix_raw_data.SubMatrix(0, num_channel, 0, wdsize);
            ...
            {
                Matrix<double> corrected = Programe.SAP(submatrix.SubMatrix(0, num_channel, 0, wdsize), wdsize / 2, 1, Math.Pow(10, -3)); //correct data
            ...
            if (Init_flag) //this line add by Hongyun and Lin, make sure does not oupt anything before correct the first window data
            {
                ...
                else
                {
                    ...
                    Matrix<double> corrected = Programe.SAP(submatrix_raw_data.SubMatrix(0, num_channel, 0, wdsize), wdsize / 2, 3, Math.Pow(10, -3)); // correct data
