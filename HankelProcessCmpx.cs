...
namespace HankelRobustDataEstimation
{
    ...
    public class hankelRobustDataEstimationComplex
    {
        //keep the past L estimated data and observed data
        private  Matrix<Complex> submatrix;               // contain the past L estimations
        private  Matrix<Complex> submatrix_raw_data;      // contains the past L raw observations        
        private  Matrix<double> flag_submatrix;          // denote whether each entry is trusted
        private  Matrix<double> flag_observed;           // denote whether each entry is observed
        private  Vector<Complex> starting_vector;         // record the measurement at the first instant
        private  Matrix<Complex> submatrix_updated;       // submatrix of last window size of data for all the channels(introduced for openECA implementation)

        private  int wdsize;//= 10;
        private  int Hankel_k;// = 6;
        private  double ratio_approx_error;// = 1.2;
        private  int tau;// = 3;
        private  int tau_0;// = 30;
        ...
        public hankelRobustDataEstimationComplex()
        {
            
        }

        //This change introduced for openECA implementation
        public void Initialize(int Meas)
        {
            ...
            tau = Convert.ToInt32(ParameterForm.a);
            tau_0 = Convert.ToInt32(ParameterForm.b);
            ...
            submatrix = Matrix<Complex>.Build.Dense(Convert.ToInt32(num_channel), wdsize + 1);
            submatrix_updated = Matrix<Complex>.Build.Dense(Convert.ToInt32(num_channel), wdsize);
            submatrix_raw_data = Matrix<Complex>.Build.Dense(Convert.ToInt32(num_channel), wdsize + 1);
            flag_submatrix = Matrix<double>.Build.Dense(Convert.ToInt32(num_channel), wdsize + 1);
            flag_observed = Matrix<double>.Build.Dense(Convert.ToInt32(num_channel), wdsize + 1);
            starting_vector = Vector<Complex>.Build.Dense(Convert.ToInt32(num_channel));
            Init_flag = false;
            instant = -300;
        ...
        public Matrix<double>[] ProcessFrame(Vector<double>[] Current_data, int numberOfFrame)
        {
            ...
            int num_untrusted = 0;
            double tau_t = Math.Max(tau, tau_0 * Math.Exp(-(numberOfFrame - instant) / decaying_factor));
            ...
            submatrix.SetColumn(wdsize, ctvector);              //add raw data vector to this although later this will be updated as submatrix gives estimated measurements at the end
            submatrix_raw_data.SetColumn(wdsize, ctvector);
            flag_observed.SetColumn(wdsize, flag_observed_ctvector);
            ...
            if (numberOfFrame >= wdsize && !Init_flag)
            {
                Matrix<Complex> corrected = Programe.SAP(submatrix.SubMatrix(0, num_channel, 0, wdsize), wdsize / 2, 1, Math.Pow(10, -3)); //correct data is provided in form of a "corrected matrix"

                for (int i = 0; i < wdsize; i++)
                {
                    submatrix.SetColumn(i, corrected.Column(i));        // reset the column of submatrix with columns of corrected matrix
                    submatrix_updated.SetColumn(i, corrected.Column(i));
                }
            ...
            if (numberOfFrame >= wdsize && Init_flag)
            {
                flag_submatrix.SetColumn(wdsize, flag_ctvector);

                Matrix<Complex> Hankel_matrix = ExtensionFunction.ExtensionFunction.Hankel(submatrix.SubMatrix(0, num_channel, 0, wdsize), Hankel_k);
                // Hankel matrix of past L observations
                Matrix<Complex> ct_kvectors = ExtensionFunction.ExtensionFunction.Hankel(submatrix.SubMatrix(0, num_channel, wdsize - Hankel_k + 1, Hankel_k), Hankel_k);
                double[] flag_ct_kvectors = (flag_submatrix.SubMatrix(0, num_channel, wdsize - Hankel_k + 1, Hankel_k)).ToColumnMajorArray();
                ...
                Matrix<Complex> coeff = (columnspace.ConjugateTranspose() * Diag_kvectors * columnspace).Inverse() *
                    columnspace.ConjugateTranspose() * Diag_kvectors * ct_kvectors;
                ...
                for (int i = 0; i < num_channel; i++)
                {
                    if (Complex.Abs(estimated_ctvector.At(i, 0) - ctvector[i]) <= tau_t * threshold)           //tau_t*threshold is s(i) in paper
                    {
                ...
                if (num_untrusted > 0)
                {
                    ...
                    coeff = (columnspace.ConjugateTranspose() * Diag_kvectors * columnspace).Inverse() *
                        columnspace.ConjugateTranspose() * Diag_kvectors * ct_kvectors;
                ...
                submatrix.SetColumn(wdsize, ctvector);
                flag_submatrix.SetColumn(wdsize, flag_ctvector);
                ...
                for (int i = 0; i < wdsize; i++)
                {
                    submatrix.SetColumn(i, submatrix.Column(i + 1));
                    submatrix_raw_data.SetColumn(i, submatrix_raw_data.Column(i + 1));
                    flag_submatrix.SetColumn(i, flag_submatrix.Column(i + 1));
                ...
                if (flag_event == 0)
                {
                    ...
                    {
                        Vector<double> consecutive_untrusted = bad_data_duration - (flag_submatrix.SubMatrix(0, num_channel,
                            wdsize - bad_data_duration + 1, bad_data_duration)).RowSums();
                        ...
                        {
                            Vector<double> column_sum = (flag_submatrix.SubMatrix(0, num_channel,
                                wdsize - bad_data_duration + 1, bad_data_duration)).ColumnSums();
                            ...
                            flag_event = wdsize - starting_index;
                ...
                {
                    flag_event--;
                    Matrix<Complex> window_data = submatrix_raw_data.SubMatrix(0, num_channel, 0, wdsize);
                    Matrix<Complex> Hankel_matrix_2 = ExtensionFunction.ExtensionFunction.Hankel(window_data, Hankel_k);
                    ...
                    for (int i = 1; i <= 500; i++)
                    {
                        int[] random_index = Enumerable.Range(0, wdsize).ToArray();
                    ...
                    {
                        submatrix = submatrix_raw_data.Clone();
                        instant = event_instant;
                        flag_submatrix = flag_observed.Clone();
                        flag_output = 1;
                    }
            ...
            {
                Matrix<Complex> corrected = Programe.SAP(submatrix.SubMatrix(0, num_channel, 0, wdsize), wdsize / 2, 1, Math.Pow(10, -3)); //correct data

                for (int i = 0; i < wdsize; i++)
                {
                    submatrix.SetColumn(i, corrected.Column(i));
            ...
            if (Init_flag) //this line add by Hongyun and Lin, make sure does not oupt anything before correct the first window data
            {
                if (flag_output == 0)
                {
                    ...
                    for(int j = 0; j < wdsize-1; j++)
                    {
                        submatrix_updated.SetColumn(j, submatrix_updated.Column(j+1));
                    }
                    submatrix_updated.SetColumn(wdsize - 1, ctvector);
                }
                else
                {
                    ...
                    Matrix<Complex> corrected = Programe.SAP(submatrix_raw_data.SubMatrix(0, num_channel, 0, wdsize), wdsize / 2, 3, Math.Pow(10, -3)); // correct data
            ...
            Matrix<double>[] submatrix_updated_real = new Matrix<double>[2];
            submatrix_updated_real[0] = Matrix<double>.Build.Dense(submatrix_updated.RowCount, submatrix_updated.ColumnCount);
            submatrix_updated_real[1] = Matrix<double>.Build.Dense(submatrix_updated.RowCount, submatrix_updated.ColumnCount);
            for (int i = 0; i < submatrix_updated.RowCount; i++)
            {
                for (int j = 0; j < submatrix_updated.ColumnCount; j++)
                {
                    submatrix_updated_real[0][i, j] = submatrix_updated.At(i, j).Magnitude;
                    submatrix_updated_real[1][i, j] = submatrix_updated.At(i, j).Phase;
                }
            }
            return submatrix_updated_real;
        }
    }
}
